      ...
      <Row className="g-4">
        {vehicles.map((vehicle) => (
          <Col md={4} key={vehicle.vehicleId}>
            <Card>
              <Card.Img variant="top" src={vehicle.imageUrl} alt={vehicle.vehicleNumber} />
              <Card.Body>
                <Card.Title>{vehicle.name}</Card.Title>
                <Card.Text>Vehicle Number: {vehicle.vehicleNumber}</Card.Text>
                <Button variant="primary" onClick={() => handleCardToggle(vehicle.vehicleId)}>
                  {expandedCard === vehicle.vehicleId ? 'Hide Details' : 'View Details'}
                </Button>

                {expandedCard === vehicle.vehicleId && (
                  <div className="mt-3">
                    <Button as={Link} to="/accident" variant="success" className="me-2">
                      Report Accident
                    </Button>
                    <Button as={Link} to="/claimForm" variant="info" className="me-2">
                      Claim Form
                    </Button>
